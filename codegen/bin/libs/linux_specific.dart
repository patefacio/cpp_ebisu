  ...
  return new File('/proc/cpuinfo')
      ...
      .map((String proc) => proc
          ...
          .map((String line) => line
              ...
              .take(1)
              .map((String word) =>
                  word.trim().toLowerCase().replaceAll(' ', '_'))
              .first)
          .toList())
      .first;
}
      ...
      ..testScenarios = [
        testScenario('catching signal', [
          given('registered handler', [
            when('signal sent', [then('signal handled')])
          ])
        ])
      ]
      ..classes = [
...
'''
    ...
    header('umask_scoped_set')
      ...
      ..testScenarios = [
        testScenario(
            'unit_test_umask_scoped_set',
            given(
                'current umask is something',
    ...
    header('linux_exceptions')
      ...
      ..classes = [
        class_('system_file_parse_exception')
          ...
          ..usings = [
            'filename_tag = boost::error_info<struct tag_filename_tag, std::string>'
          ]
          ..bases = [
            base('boost::exception')
              ..isVirtual = true
              ..cppAccess = public,
            base('std::exception')
              ..isVirtual = true
    ...
    header('cpu_info')
      ...
      ..classes = [
        class_('processor')
          ...
          ..usesStreamers = true
          ..usingsPostDecl = [
            using('Processor_list', 'std::vector< Processor >')
          ]
          ..memberCtors = [
            memberCtor(['processor', 'proc_map'])
          ]
          ..withCustomBlock(clsPublic, (CodeBlock cb) {
            cb.snippets.add(br(processorEntries.map((String entry) {
              return '''
...
'''
          ...
          ..isSingleton = true
          ..withDefaultCtor(
              (defaultCtor) => defaultCtor..includesProtectBlock = true)
          ..customBlocks = [clsPrivate]
          ..members = [
            member('processors')
              ..doc = 'List of processors'
              ..type = #u.processor_list
              ..isByRef = true
              ..access = ro
          ],
      ]
