...
struct Conversion_logger {
  std::shared_ptr<spdlog::logger> & logger() {
    static std::shared_ptr<spdlog::logger> logger = spdlog::stdout_logger_mt("conversion");
    return logger;
  }
};
...
namespace {
  ...
  std::shared_ptr<spdlog::logger> conversion_logger { Conversion_logger<int>().logger() };
}
