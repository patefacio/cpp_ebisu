#ifndef __EBISU_ORM_OTL_UTILS_HPP__
...
// custom <FcbPreNamespace otl_utils>
...
inline std::ostream& operator<<(std::ostream& out, otl_datetime const& t) {
  out << t.month << '/' << t.day << '/' << t.year << ' ' << t.hour << ':'
      << t.minute << ':' << t.second;
  return out;
}
...
// end <FcbPreNamespace otl_utils>

namespace ebisu {
namespace orm {
// custom <FcbEndNamespace otl_utils>
...
inline std::string sql_quote(
    typename ebisu::utils::Fixed_size_char_array<ARRAY_SIZE> const& value) {
  return sql_quote(std::string(value));
}

inline std::string sql_quote(otl_long_string const& value) {
  return sql_quote(std::string(reinterpret_cast<char const*>(value.v)));
}

template <typename T>
inline boost::posix_time::ptime otl_type_to_boost_ptime(T const& o) {
  return boost::posix_time::ptime(
      boost::gregorian::date(o.year, o.month, o.day),
      boost::posix_time::hours(o.hour) + boost::posix_time::minutes(o.minute) +
          boost::posix_time::seconds(o.second));
}

inline boost::posix_time::ptime otl_datetime_to_boost_ptime(
    otl_datetime const& o) {
  return otl_type_to_boost_ptime<otl_datetime>(o);
}

inline boost::posix_time::ptime otl_time_to_boost_ptime(otl_time const& o) {
  return otl_type_to_boost_ptime<otl_time>(o);
}

template <typename T>
inline boost::posix_time::time_duration otl_type_to_boost_time_of_date(
    T const& o) {
  return boost::posix_time::time_duration(boost::posix_time::hours(o.hour) +
                                          boost::posix_time::minutes(o.minute) +
                                          boost::posix_time::seconds(o.second));
}

inline boost::posix_time::time_duration otl_datetime_to_boost_time_of_day(
    otl_datetime const& o) {
  return otl_type_to_boost_time_of_date<otl_datetime>(o);
}

inline boost::posix_time::time_duration otl_time_to_boost_time_of_day(
    otl_time const& o) {
  return otl_type_to_boost_time_of_date<otl_time>(o);
}

template <typename T>
inline void boost_ptime_to_otl_type(boost::posix_time::ptime const& t, T& o) {
...
inline otl_datetime boost_ptime_to_otl_datetime(
...
inline otl_time boost_ptime_to_otl_time(boost::posix_time::ptime const& t) {
  otl_time result;
  boost_ptime_to_otl_type(t, result);
  return result;
}

inline otl_datetime boost_date_to_otl_datetime(
...
inline std::string create_select_union(INPUT_ITERATOR begin, INPUT_ITERATOR end,
  ...
  if (begin != end) {
    ...
    result << "select "
           << sql_quote<typename INPUT_ITERATOR::value_type>(*begin) << " as "
           << name;
    for (++begin; begin != end; ++begin) {
      result << "\n  union select "
             << sql_quote<typename INPUT_ITERATOR::value_type>(*begin);
    }
    if (wrapped) {
      result << ')';
    }
...
#endif  // __EBISU_ORM_OTL_UTILS_HPP__
