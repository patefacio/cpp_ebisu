#ifndef __EBISU_ORM_CODE_METRICS_TABLE_CODE_LOCATIONS_HPP__
#define __EBISU_ORM_CODE_METRICS_TABLE_CODE_LOCATIONS_HPP__
...
struct Code_locations_value {
  bool operator==(Code_locations_value const& rhs) const {
    return this == &rhs ||
           (code_packages_id == rhs.code_packages_id && label == rhs.label &&
            file_name == rhs.file_name && line_number == rhs.line_number &&
  ...
  ebisu::utils::Fixed_size_char_array<256> label;
  ebisu::utils::Fixed_size_char_array<256> file_name;
  int32_t line_number;
  ebisu::utils::Fixed_size_char_array<40> git_commit;
};

inline otl_stream& operator<<(otl_stream& out,
                              Code_locations_value const& value) {
  out << value.code_packages_id << value.label << value.file_name
      << value.line_number << value.git_commit;
  return out;
}

inline otl_stream& operator>>(otl_stream& src, Code_locations_value& value) {
  src >> value.code_packages_id >> value.label >> value.file_name >>
      value.line_number >> value.git_commit;
  return src;
}
...
class Code_locations {
 public:
  ...
  static void print_recordset_as_table(Row_list_t const& recordset,
                                       std::ostream& out) {
    ebisu::orm::print_recordset_as_table<Code_locations>(recordset, out);
  }

  static void print_values_as_table(Value_list_t const& values,
                                    std::ostream& out) {
    ebisu::orm::print_values_as_table<Code_locations>(values, out);
  }
...
inline void link_rows(Code_locations<>::Row_t& from_row,
...
#endif  // __EBISU_ORM_CODE_METRICS_TABLE_CODE_LOCATIONS_HPP__
